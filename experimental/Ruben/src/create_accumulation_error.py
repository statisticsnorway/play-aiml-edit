...
class AccumulationErrors:
    def __init__(self, cfg, years, type_of_errors, total_error_prct, seed=42):
        self.cfg = cfg
        self.years = years
    ...
    def create_accumulation_errors(self, df):
        ...
        orgs_with_errors = self.rng.choice(
            unique_orgs,
        ...
        result_df[self.cfg.error_col] = (
            result_df[self.cfg.omsetning] != result_df[self.original_value]
        ).astype(int)
    ...
    def _apply_errors_to_org(self, df, org):
        ...
        if self.cfg.multiple_error_years:
            years_with_errors = [
                ...
                if self.rng.random() < self.cfg.error_year_probability
            ]
            if len(years_with_errors) == 0:
                years_with_errors = [self.rng.choice(available_years)]
        else:
            years_with_errors = [self.rng.choice(available_years)]
    ...
    def _apply_error_to_year(self, df, org, org_data, year):
        ...
        start_idx = self._get_start_idx_in_year(year_data, full_org_data=org_data)

        months_left_in_year = len(year_data) - (
            start_idx - year_data.index.get_loc(year_data.index[0])
        )
        max_duration = min(self.cfg.max_duration, months_left_in_year)

        if max_duration < 3:
            return df

        duration = self.rng.integers(
            self.cfg.min_duration, max_duration + 1
        )
        ...
        error_type = self.rng.choice(self.type_of_errors)

        original_values = df.loc[error_indices, self.original_value].values

        if error_type == 'cumulative_sum':
            modified_values = self._cumulative(values=original_values)
        elif error_type == 'cascading':
            modified_values = self._cascading(values=original_values)
        elif error_type == 'random':
            modified_values = self._random(values=original_values, error_probability=0.5)
    ...
    def _get_start_idx_in_year(self, year_data, full_org_data):
        use_preferred_month = self.rng.random() < self.cfg.start_month_prob
        ...
        return self.rng.integers(0, max_start + 1) if max_start > 0 else 0
